...
local function IsReadyToParry(loliBall) --// to determine should we parry or not
    if character:FindFirstChild("Highlight") then
        ...
        if Distance < 40 and math.abs(loliBall.Velocity.Magnitude) >= 40 then
            parryButton:Fire();
            print("Close")
...
getgenv().HeartbeatConnection = game:GetService("RunService").Heartbeat:Connect(function(v)
    ...
    if getgenv().Settings["High Position"] then
        myteam = tostrintg(LocalPlayer.Team)
        if myteam == "Playing" or myteam == "Red" or myteam == "Blue" or myteam == "Yellow" or myteam == "Green" then
            local cframe = currentMap.BALLSPAWN.Position
            LocalPlayer.Character.HumanoidRootPart.Position = cframe + Vector3.new(0, 45, 0)
        end
    end
    if ball then
        -- Detects spam case
