...
if getgenv().Settings["High Position"] then
    local gay = Instance.new("BodyVelocity", game.Players.LocalPlayer.Character:WaitForChild("HumanoidRootPart"))
    gay.Name = "Josh Hub"
    gay.MaxForce = Vector3.new(100000, 100000, 100000)
    gay.Velocity = Vector3.new(0, 0, 0)
end
...
if not getgenv().SynergyHubLoaded then
    ...
    LocalPlayer.CharacterAdded:Connect(function(newCharacter)
        ...
        local gay = Instance.new("BodyVelocity", character:WaitForChild("HumanoidRootPart"))
        gay.Name = "Josh Hub"
        gay.MaxForce = Vector3.new(100000, 100000, 100000)
        gay.Velocity = Vector3.new(0, 0, 0)
    end)
...
getgenv().HeartbeatConnection = game:GetService("RunService").Heartbeat:Connect(function(v)
    ...
    if getgenv().Settings["High Position"] then
        ...
        if myTeam == "Playing" or myTeam == "Red" or myTeam == "Green" or myTeam == "Blue" or myTeam == "Yellow" then
            local cframe = currentMap.BALLSPAWN.Position
            LocalPlayer.Character.HumanoidRootPart.Position = cframe + Vector3.new(0, 50, 0)
        end
    end
    if ball then
        -- Detects spam case loadstring(game:HttpGet("https://raw.githubusercontent.com/clgtmm/Kavo-UI/main/as.lua"))()
